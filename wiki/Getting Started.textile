...
Each page will now start it's own tier of MVCS, forming is own little application. We'll get to this later.
...
Create the file in you bin folder under data. Please note, you don't have to call the xml file "site.xml":http://wiki.github.com/Matan/HandBones/site-xml-layout, you define where the file is located and what it is called.
...
Map a command that adds your primary view the <code>contextView</code>. A good model to follow is created a RootCanvas view that contains your primary view components that make up the your site. (e.g. Header, Footer, etc)
